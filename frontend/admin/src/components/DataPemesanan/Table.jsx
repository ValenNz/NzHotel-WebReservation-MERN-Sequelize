...
const Table = () => {
  ...
  const handleCari = () => {
    ...
    axios
      .post(`http://localhost:8000/pemesanan/search`, data, {
        headers: {
  ...
  const handleSearchByName = () => {
    ...
    axios
      .post(`http://localhost:8000/pemesanan/findByNamaTamu`, data, {
        headers: {
          Authorization: "Bearer " + sessionStorage.getItem("token"),
        },
      })
  ...
  function Delete(id) {
    let url = "http://localhost:8000/pemesanan/" + id;
          ...
          {pemesanan?.map((pemesanan, index) => (
            <tr key={pemesanan.id_pemesanan}>
              ...
              <td className="p-4">
                {/* Tombol status */}
                <button
                  className={`bg-[#faa935] p-3 rounded-md ${
